...
> Scope: **Current host safety architecture, with RC5 operation recovery additions.** Source facts, observations and inference are distinguished below. Exact test status belongs in [Test Evidence](RC5_TEST_EVIDENCE.md); installer acceptance belongs in [Validation](../VALIDATION.md).
...
M_Bamboo policy: a confirmed LDC transport fault is first-class safety evidence. It must taint subsequent Eddy data, latch an Eddy communication fault, abort an active Eddy probe where safely possible, invalidate Z trust, and block further Eddy-dependent Z operations until the required transport and Z trust have been re-established. Faults requiring restart remain blocked; a historical error alone does not permanently prevent validated recovery.
...
- Z invalidation remains necessary because geometry/no-trigger failures can occur with healthy I2C. The shipped `homing.py` is an audit reference, not a file deployed by this installer; do not infer deployment from its presence.
...
No. MCU modification, rebuilding and flashing are outside this project unless the user explicitly changes that boundary. The design is to consume Sovol's existing asynchronous I2C error report, decode it at the lowest host-side hardware layer, pass a structured fault to `probe_eddy_current.py`, and reuse Klipper's existing trsync/homing machinery to stop and invalidate unsafe operations. MCU-side defects are documented for transparency, while the release remains a user-space/Klipper modification.
...
No. On this Sovol STM32F1 driver, BUSY is commonly additional bus-state evidence observed after the transaction has already failed due to NACK/TIMEOUT. One `NACK | BUSY` is sufficient to invalidate the **current transaction**, but it is not proof of permanent sensor-hardware failure. EC2 therefore stops/taints/aborts the active action and revokes Z trust for bed-facing Z contexts, then permits the no-motion `M_BAMBOO_EDDY_RECOVERY_CHECK`. Repeated valid LDC identity reads with no new `transport_fault_seq` can restore transport trust. When Z recovery is required, Z remains untrusted and one fresh Safe Home is armed. PREARM faults caught before any coordinate invalidation may preserve Z trust.

Recovery must not launch from asynchronous fault callbacks. A supported outer synchronous operation can recover after failed work has terminalized; that is distinct from retrying an active sensor transaction. PREARM may also absorb a transient using bounded no motion readiness checks before motion begins. Once probing is active, a new fault still aborts the original transaction. See the operation recovery questions below.
...
### Historical hardware results, August 2026

After the pre-arm gate was introduced, hardware validation covered repeated G28/Safe Home cycles, direct contact probing, repeated `CLEAN_NOZZLE`, repeated Z-offset calibration, QGL, adaptive rapid mesh, final XY re-home, and a full slicer-driven `START_PRINT` followed by an actual cube print and post-print status inspection. In that recorded post-print session, `M_BAMBOO_EDDY_STATUS` reported **30 pre-arm checks with 0 transport faults, 0 transient recoveries, 0 pre-arm failures, 0 forced stream quarantines, and 0 repeated-fault suppressions**.

This is healthy path compatibility evidence from that session. It does not establish a causal reduction in fault incidence or a reliable minimum dwell time. Later churn evidence did not support a universal 50/100 ms threshold; PREARM remains valuable because it rejects an unhealthy start. See [Test Evidence](RC5_TEST_EVIDENCE.md) for the later experiments and corrected hypotheses.
...
## What does RC5 add above the existing safety core?

SR1 coordinates the existing CLEAN, PRE_ZCAL, QGL, G28 Z, BED_MESH and POST_ZCAL stages. RS1 retires rapid scan sessions so queued lookahead callbacks cannot dereference a released gather object. GR1 adds one outer synchronous recovery owner for G28, RUN_PROBE_VIR_CONTACT, CLEAN_NOZZLE, Z_OFFSET_CALIBRATION, QUAD_GANTRY_LEVEL and BED_MESH_CALIBRATE. BASE commands and arbitrary calibration commands are not implicitly covered.

The safety label remains unchanged; the exact backend hash in the [Version Map](../VERSION_MAP.md) identifies the combination. No healthy rapid scan speed, height, path, sample window, timing or interpolation change is part of RS1.

## When does recovery repeat an operation, and when must it stop?

Eligibility requires new `transport_fault_seq` or `preflight_failed_count` evidence during the owner invocation. Ordinary errors are propagated. Failed work must have ended before no motion transport checks and, if required, fresh Safe Home rebuild trust. The entire failed operation is replayed once, never only a failed QGL point or partial mesh.

During START_PRINT, SR1 remains owner and nested GR1 wrappers pass through. Each stage invocation permits one recovery; the START total is three independent episodes. A second fault before clean completion or recovery failure terminates. Async I2C, bulk, lookahead and flush callbacks never initiate workflow recovery. A successful recovery does not validate the discarded data.

See [Recovery Supervisor Design](RC5_RECOVERY_SUPERVISOR_DESIGN.md) and [START Design](RC5_START_RECOVERY_DESIGN.md) for implementation detail; [Command Reference](COMMAND_REFERENCE.md) defines interfaces.

## Why retain the current START_PRINT sequence and contact threshold?

Contact verification has shown greater spread than the observed Eddy Z home triggers. A failure of the legacy adjacent sample 0.020 mm rule without advancing transport markers is not evidence of an I2C fault. A noisy contact measurement also cannot establish a smaller mechanical drift reliably. Rapid mesh sag and a need for inferred compensation remain unproven.

Keep the current two stage calibration, PREARM and ZCAL threshold pending better evidence. The coordinator cleans temporary startup state and owns mesh stage settings; it does not replace the underlying measurement algorithms. See the [test plan](RC5_TEST_PLAN.md) for the required contact series and coordinate transform controls.

## Where is the lower layer source audit?

The [Sovol I2C source audit](I2C_ROOT_CAUSE_AND_HOST_BOUNDARY.md) retains the detailed driver history, BUSY pin lookup issue and failed transaction boundary findings. It explains the limitations the host can contain and the MCU behavior this project does not modify.

## Is PLR part of the current candidate?

No. PLR remains deferred. The stock Sovol resume design relies on coordinate fabrication and command-line text matching that do not meet the current coordinate-trust and checkpoint-identity requirements. Normal printing does not depend on the deferred PLR feature.

## Does the installer implement downgrade?

There is no generic downgrade engine. The intended path is a validated Full Restore followed by the target historical installer. The current development candidate has an open config restore defect; the path is not yet qualified for that candidate. See [Deployment & Restore](DEPLOYMENT_AND_ROLLBACK.md).

## What should survive Full Restore?

The contract preserves unrelated user cfg content and SAVE_CONFIG, removes/reverses owned blocks, restores original backends from `extras/mb_bak/`, and deletes files recorded as originally absent. This is the required contract, not a claim that the current candidate has met it. The [offline findings](RC5_OFFLINE_INSTALLER_FINDINGS.md) document the remaining START_PRINT block.
