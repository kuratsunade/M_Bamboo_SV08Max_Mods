...
class Homing:
    ...
    def _invalidate_failed_z_home(self, homing_axes):
        if 2 not in homing_axes:
            return
        kin = self.toolhead.get_kinematics()
        if hasattr(kin, 'note_z_not_homed'):
            kin.note_z_not_homed()
            self.gcode.respond_info(
                "MBZSAFE: failed Z homing invalidated Z homing state")
    def home_rails(self, rails, forcepos, movepos):
        ...
        try:
            ...
            self._invalidate_failed_z_home(homing_axes)
            raise
        # Perform second home
        if hi.retract_dist:
            # Retract
...
class PrinterHoming:
    ...
    def manual_home(self, toolhead, endstops, pos, speed,
        ...
        try:
            ...
            raise
    def _invalidate_z_after_probe_failure(self):
        toolhead = self.printer.lookup_object('toolhead')
        kin = toolhead.get_kinematics()
        eventtime = self.printer.get_reactor().monotonic()
        homed_before = toolhead.get_status(eventtime).get('homed_axes', '')
        if hasattr(kin, 'note_z_not_homed'):
            kin.note_z_not_homed()
        homed_after = toolhead.get_status(eventtime).get('homed_axes', '')
        self.gcode.respond_info(
            "MBZSAFE: probe failure invalidated Z homing state "
            "homed_axes=%s -> %s"
            % (homed_before or '<none>', homed_after or '<none>'))
    ...
    def probing_move(self, mcu_probe, pos, speed, non_contact_probe=True):
        ...
        try:
            ...
            self._invalidate_z_after_probe_failure()
            if self.printer.is_shutdown():
                raise self.printer.command_error(
