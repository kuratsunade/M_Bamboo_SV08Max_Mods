...
class MBambooSafeHoming:
    ...
    def _home_z_after_clearance(self, gcmd):
        ...
        # Healthy-path transport readiness gate.  It performs only no-motion
        # LDC identity reads and bounded settle/retry; if transport is unstable
        # the Z-home action is rejected before any bed-facing Z movement.
        probe_obj = self.printer.lookup_object('probe', None)
        if (probe_obj is not None
                and hasattr(probe_obj, 'preflight_safe_home_z')):
            probe_obj.preflight_safe_home_z()
