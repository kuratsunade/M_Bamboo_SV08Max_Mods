...
class EddyEndstopWrapper:
    ...
    def __init__(self, config, sensor_helper, calibration):
        ...
        # Pre-arm transport quiescence.  A transport failure observed here is
        # intentionally handled before any bed-facing Z motion starts.  The
        # callback still preserves fault evidence and statistics, but a bounded
        # sequence of clean identity reads may prove the bus recovered without
        # invalidating an otherwise trusted Z coordinate.
        self._preflight_active = False
        self._preflight_context = None
        self._preflight_fault_seen = False
        self._preflight_last_fault = {}
        self._z_recovery_required = False
        self._preflight_check_count = 0
        self._preflight_transient_recovered_count = 0
        self._preflight_failed_count = 0
        # Session diagnostics.  Keep transport history separate from current
        ...
        self._last_transport_fault = {}
        self._recovery_check_count = 0
        self._recovery_check_pass_count = 0
        self._armed_recovery_success_count = 0
    ...
    def _decode_transport_type(self, evidence):
    ...
    def _decode_transport_reason(self, evidence):
        return '%s raw=%s seq=%s' % (
            self._decode_transport_type(evidence),
            evidence.get('err_code'), evidence.get('seq'))

    def _transport_fault_context(self):
        if self._preflight_active:
            return 'PREARM:%s' % (self._preflight_context or 'UNKNOWN')
        tx = self._active_transaction
        if tx is not None:
            return tx.get('caller') or tx.get('kind') or 'TRANSACTION'
        if self._active_scan_session is not None:
            return 'BED_MESH_SCAN'
        return 'IDLE'

    def _record_transport_fault(self, evidence):
        seq = int(evidence.get('seq') or 0)
        if seq <= self._last_counted_transport_fault_seq:
            return
        self._last_counted_transport_fault_seq = seq
        fault_type = self._decode_transport_type(evidence)
        context = self._transport_fault_context()
        self._transport_fault_count += 1
        self._transport_fault_type_counts[fault_type] = (
            self._transport_fault_type_counts.get(fault_type, 0) + 1)
        self._transport_fault_context_counts[context] = (
            self._transport_fault_context_counts.get(context, 0) + 1)
        record = dict(evidence)
        record['type'] = fault_type
        record['context'] = context
        record['recorded_time'] = self._printer.get_reactor().monotonic()
        self._last_transport_fault = record
    ...
    def _handle_transport_fault(self, evidence):
        ...
        seq = int(evidence.get('seq') or 0)
        if seq <= self._last_handled_transport_fault_seq:
            # _consume_pending_transport_fault() may deliberately consume the
            # monotonic sequence before this reactor callback runs.  The later
            # callback is the same evidence, not a second fault; processing it
            # again could incorrectly re-latch a pre-arm fault after recovery.
            self._record_transport_fault(evidence)
            return
        self._last_handled_transport_fault_seq = seq
        self._record_transport_fault(evidence)
        if self._preflight_active and self._active_transaction is None:
            self._preflight_fault_seen = True
            self._preflight_last_fault = dict(evidence)
            self._diag(0,
                       'PREARM transport transient observed context=%s '
                       'reason=%s; no Z motion started, holding motion and '
        ...
        self._safe_home_recovery_token = False
        if tx is not None:
            if tx.get('kind') in ('HOMING', 'PROBE'):
                self._z_recovery_required = True
    ...
    def _consume_pending_transport_fault(self):
        ...
        if self._preflight_active and self._active_transaction is None:
            ...
            self._preflight_last_fault = dict(evidence)
            self._diag(0,
                       'PREARM pending transport transient observed '
                       'context=%s reason=%s; no Z motion started, holding '
    ...
    def _recovery_guidance(self):
        ...
        if self._transport_state == 'TRANSPORT_FAULT':
            if not self._z_recovery_required:
                return ('The transport is not ready, but this fault was caught '
                        'before bed-facing Z motion started, so existing Z '
                        'trust was not invalidated by the pre-arm gate. Run '
                        'M_BAMBOO_EDDY_RECOVERY_CHECK. If it reports RECOVERED, '
                        'normal operations may resume without a recovery G28; '
                        'otherwise use FIRMWARE_RESTART and inspect the Eddy/I2C '
                        'connection.')
    ...
    def preflight_transport_ready(self, context='EDDY_MOTION'):
        # Do not silently bypass an already-latched failure.  Armed Safe Home
        # recovery has its own explicit authorization path and has already
        # passed M_BAMBOO_EDDY_RECOVERY_CHECK.
        self._consume_pending_transport_fault()
        if self._fault_state is not None:
            self._check_fault()
        if self._active_transaction is not None:
            raise self._printer.command_error(
                'M_Bamboo Eddy Safety: transport preflight requested while an '
                'Eddy transaction is active')
        checker = getattr(self._sensor_helper, 'check_transport_health', None)
        if checker is None:
            return True
        self._preflight_check_count += 1
        entry_seq = self._transport_seq()
        attempts = (
            (0.025, 2, 0.025),
            (0.075, 2, 0.025),
            (0.125, 2, 0.025),
        ...
        self._preflight_last_fault = {}
        results = []
        clean_after_failure = 0
        try:
            for attempt_no, (settle, reads, gap) in enumerate(attempts, 1):
                result = checker(
                    settle_time=settle, read_count=reads, read_gap=gap)
                results.append(dict(result))
                # Consume a serial-thread sequence that may have arrived before
                # its reactor callback.  While preflight_active this records
                # evidence but deliberately does not latch a motion fault.
                self._consume_pending_transport_fault()
                current_seq = self._transport_seq()
                clean = bool(result.get('ok') and
                             result.get('seq_end') == current_seq)
                if clean:
                    if self._preflight_fault_seen or len(results) > 1:
                        clean_after_failure += 1
                        if clean_after_failure < 2:
                            self._diag(1, 'PREARM context=%s attempt=%d clean; '
                                       'requiring one more clean window after '
                                       'transient/failed readiness check'
                                       % (context, attempt_no))
                            continue
                    if current_seq > entry_seq:
                        marker = getattr(
                            self._sensor_helper,
                            'mark_transport_recovered_through', None)
                        if marker is not None:
                            marker(current_seq)
                        self._preflight_transient_recovered_count += 1
                        self._diag(0, 'PREARM RECOVERED context=%s fault_seq=%d->%d; '
                                   'transport stable before motion, Z trust '
                                   'unchanged' % (context, entry_seq, current_seq))
                    else:
                        self._diag(2, 'PREARM READY context=%s attempt=%d seq=%d'
                                   % (context, attempt_no, current_seq))
                    self._transport_state = 'HEALTHY'
                    return True
                clean_after_failure = 0
                self._diag(1, 'PREARM NOT READY context=%s attempt=%d/%d '
                           'seq=%s->%s reads=%d; motion remains held'
                           % (context, attempt_no, len(attempts),
                              result.get('seq_start'), result.get('seq_end'),
                              len(result.get('reads', ()))))
        finally:
            self._preflight_active = False
            self._preflight_context = None
        self._preflight_failed_count += 1
        self._z_recovery_required = False
        self._transport_state = 'TRANSPORT_FAULT'
        self._recovery_authorized = False
        self._safe_home_recovery_token = False
        last = results[-1] if results else {}
        evidence = self._preflight_last_fault
        if evidence:
            reason = 'PREARM_NOT_READY|' + self._decode_transport_reason(evidence)
        else:
            reason = ('PREARM_NOT_READY identity/readiness check failed '
                      'seq=%s->%s' %
                      (last.get('seq_start'), last.get('seq_end')))
        self._set_fault('HARD_COMM_FAULT', reason)
        raise self._printer.command_error(
            'M_Bamboo Eddy Safety: transport was not stable before %s; no '
            'bed-facing Z motion was started. %s'
            % (context, self._recovery_guidance()))

    def run_transport_recovery_check(self, gcmd):
        self._consume_pending_transport_fault()
        self._recovery_check_count += 1
        ...
        if result.get('ok') and after == before:
            self._recovery_check_pass_count += 1
            ...
            if not self._z_recovery_required:
                ...
                self._fault_reason = None
                marker = getattr(
                    self._sensor_helper, 'mark_transport_recovered_through', None)
                if marker is not None:
                    marker(after)
                gcmd.respond_info(
                    'MBEDDY RECOVERY: RECOVERED. LDC identity reads=[%s], '
                    'fault_seq=%d unchanged. The fault was caught before '
                    'bed-facing Z motion, so existing Z trust is unchanged '
                    'and normal operations may resume.' % (read_text, after))
                return
    ...
    def _complete_recovery_success(self, tx):
        ...
        self._restart_required = False
        self._z_recovery_required = False
        self._armed_recovery_success_count += 1
        marker = getattr(
            self._sensor_helper, 'mark_transport_recovered_through', None)
        if marker is not None:
            marker(self._transport_seq())
    ...
    def get_diagnostic_report(self):
        ...
        lines = [
            ...
            'Transport trust watermark/current: %s / %s' % (
                self._raw_diag().get('transport_trusted_through_seq', 0),
                self._transport_seq()),
            ...
            'Transport fault contexts: %s' % (
                self._format_counts(self._transport_fault_context_counts),),
            'Pre-arm checks: %d; transient recoveries: %d; failures: %d' % (
                self._preflight_check_count,
                self._preflight_transient_recovered_count,
                self._preflight_failed_count),
            'Recovery checks: %d/%d passed; armed recovery successes: %d' % (
                self._recovery_check_pass_count, self._recovery_check_count,
                self._armed_recovery_success_count),
            'Recovery armed: %s' % (
            ...
            'Last transport fault: %s' % (
                ('seq=%s type=%s context=%s' % (
                    self._last_transport_fault.get('seq'),
                    self._last_transport_fault.get('type'),
                    self._last_transport_fault.get('context')))
                if self._last_transport_fault else 'None'),
            'LDC last-error telemetry (historical; transport state above is current): %s' % (
                self._raw_diag_text(),),
        ]
...
class PrinterEddyProbe:
    ...
    def preflight_safe_home_z(self):
        # A latched transport fault has already gone through the explicit
        # recovery-check path; do not run the normal healthy-path preflight
        # before consuming its one-shot Safe Home recovery authorization.
        if self.mcu_probe._fault_state is not None:
            return False
        return self.mcu_probe.preflight_transport_ready('SAFE_HOME_Z')
    ...
    def start_probe_session(self, gcmd):
        ...
        self.mcu_probe.preflight_transport_ready(
            'BED_MESH_SCAN' if method in ('scan', 'rapid_scan')
            else 'PROBE_SESSION')
    ...
    def run_non_contact_calibrate(self, gcmd, internal_endstop_offset, z_hop_speed=5.):
        self.mcu_probe._check_fault()
        self.mcu_probe.preflight_transport_ready('EDDY_CALIBRATION')
        toolhead = self.printer.lookup_object("toolhead")
        ## set z zero
        pos = toolhead.get_position()
        pos[2] = 0.
    ...
    def run_contact_probe(self, gcmd):
        self.mcu_probe._check_fault()
        self.mcu_probe.preflight_transport_ready('CONTACT_PROBE')
