...
class ZoffsetCalibration:
    ...
    def cmd_Z_OFFSET_CALIBRATION(self, gcmd):
        ...
        # Explicit final X/Y mechanical reseat for the post-mesh calibration.
        # This path never homes or relabels Z; it only raises Z if needed,
        # homes X and Y through upstream Klipper homing, and verifies dZ.
        rehome_xy = gcmd.get_int("REHOME_XY", 0, minval=0, maxval=1)
        rehome_xy_z_tolerance = gcmd.get_float(
            "REHOME_XY_Z_TOLERANCE", 0.02, minval=0., maxval=0.25)
        ...
        if use_current_z:
            # >>> Master_Bamboo: ZCAL_FINAL_XY_REHOME BEGIN >>>
            if rehome_xy:
                # Explicit final mechanical XY reseat.  Z has already been
                # validated as homed above.  Only positive-Z clearance and
                # raw X/Y homing are permitted here; never home or relabel Z.
                clearance = self.z_hop if self.z_hop > 0. else 5.0
                clearance_speed = (self.z_hop_speed
                                     if self.z_hop_speed > 0. else 5.0)
                before_rehome = self.toolhead.get_position()
                dbg("FINAL_XY_REHOME requested homed=%s "
                    "XYZ=(%.3f, %.3f, %.3f) clearance=%.3f z_tol=%.3f"
                    % (status['homed_axes'], before_rehome[0],
                       before_rehome[1], before_rehome[2], clearance,
                       rehome_xy_z_tolerance))

                if before_rehome[2] < clearance:
                    dbg("FINAL_XY_REHOME Z clearance %.3f -> %.3f"
                        % (before_rehome[2], clearance))
                    self.toolhead.manual_move(
                        [None, None, clearance], clearance_speed)
                    dbg("FINAL_XY_REHOME Z clearance done Z=%.3f"
                        % (self.toolhead.get_position()[2],))

                def invalidate_z_and_abort(message):
                    kin = self.toolhead.get_kinematics()
                    if hasattr(kin, 'note_z_not_homed'):
                        kin.note_z_not_homed()
                    raise gcmd.error(message)

                z_before_x = self.toolhead.get_position()[2]
                dbg("FINAL_XY_REHOME raw X home start Z=%.6f"
                    % (z_before_x,))
                gcmd_G28_X = self.gcode.create_gcode_command(
                    "G28", "G28", {'X': 0})
                raw_homing.cmd_G28(gcmd_G28_X)
                after_x = self.toolhead.get_position()
                dz_x = after_x[2] - z_before_x
                dbg("FINAL_XY_REHOME raw X home done dZ=%.6f "
                    "XYZ=(%.3f, %.3f, %.3f)"
                    % (dz_x, after_x[0], after_x[1], after_x[2]))
                if abs(dz_x) > rehome_xy_z_tolerance:
                    invalidate_z_and_abort(
                        'ZoffsetCalibration: REHOME_XY X homing changed Z by '
                        '%.6f mm; Z reference invalidated.' % (dz_x,))
                status_after_x = self.toolhead.get_status(
                    self.printer.get_reactor().monotonic())
                if 'z' not in status_after_x['homed_axes']:
                    raise gcmd.error(
                        'ZoffsetCalibration: REHOME_XY lost Z homing state '
                        'after X home.')

                z_before_y = self.toolhead.get_position()[2]
                dbg("FINAL_XY_REHOME raw Y home start Z=%.6f"
                    % (z_before_y,))
                gcmd_G28_Y = self.gcode.create_gcode_command(
                    "G28", "G28", {'Y': 0})
                raw_homing.cmd_G28(gcmd_G28_Y)
                after_y = self.toolhead.get_position()
                dz_y = after_y[2] - z_before_y
                dbg("FINAL_XY_REHOME raw Y home done dZ=%.6f "
                    "XYZ=(%.3f, %.3f, %.3f)"
                    % (dz_y, after_y[0], after_y[1], after_y[2]))
                if abs(dz_y) > rehome_xy_z_tolerance:
                    invalidate_z_and_abort(
                        'ZoffsetCalibration: REHOME_XY Y homing changed Z by '
                        '%.6f mm; Z reference invalidated.' % (dz_y,))
                status_after_y = self.toolhead.get_status(
                    self.printer.get_reactor().monotonic())
                final_homed = status_after_y['homed_axes']
                if ('x' not in final_homed or 'y' not in final_homed or
                        'z' not in final_homed):
                    raise gcmd.error(
                        'ZoffsetCalibration: REHOME_XY did not preserve xyz '
                        'homing state.')
                final_pos = self.toolhead.get_position()
                dbg("FINAL_XY_REHOME verified homed=%s "
                    "XYZ=(%.3f, %.3f, %.3f)"
                    % (final_homed, final_pos[0], final_pos[1],
                       final_pos[2]))
                status = status_after_y
                current_pos = final_pos
            else:
                # Existing behavior is preserved when REHOME_XY is omitted.
                if current_pos[2] < 5:
                ...
                if ('x' not in status['homed_axes'] or
                    ...
                    dbg("raw XY home done XYZ=(%.3f, %.3f, %.3f) dZ=%.6f"
                        % (after[0], after[1], after[2],
                           after[2] - before[2]))
            # <<< Master_Bamboo: ZCAL_FINAL_XY_REHOME END <<<
