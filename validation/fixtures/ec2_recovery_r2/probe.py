...
class ProbeSessionHelper:
    ...
    def start_probe_session(self, gcmd):
        ...
        self.multi_probe_pending = True
        self.results = []
        self.probe_caller = gcmd.get_command() or 'UNKNOWN'
        return self
    ...
    def _probe(self, speed, non_contact_probe=True):
        ...
        pos = toolhead.get_position()
        original_target_z = self.z_position
        target_z = original_target_z
        safety_context = {}
        if non_contact_probe and hasattr(self.mcu_probe, 'bound_probe_target'):
            target_z, safety_context = self.mcu_probe.bound_probe_target(
                pos[2], original_target_z)
        if hasattr(self.mcu_probe, 'set_probe_context'):
            self.mcu_probe.set_probe_context(
                self.probe_caller, original_target_z, target_z,
                safety_context)
        pos[2] = target_z
        try:
    ...
    def run_probe(self, gcmd):
        if not self.multi_probe_pending:
            self._probe_state_error()
        # NON_CONTACT_PROBE may originate from an internally-created
        # GCodeCommand with a Python bool.  GCodeCommand.get() normally
        # applies str(), so False becomes the truthy string "False".
        # Normalize explicitly before selecting the safety envelope and
        # the contact/non-contact probing path.
        raw_non_contact = gcmd.get('NON_CONTACT_PROBE', default='1')
        normalized = str(raw_non_contact).strip().lower()
        if normalized in ('1', 'true', 'yes', 'on'):
            non_contact_probe = True
        elif normalized in ('0', 'false', 'no', 'off'):
            non_contact_probe = False
        else:
            raise gcmd.error(
                "NON_CONTACT_PROBE must be one of "
